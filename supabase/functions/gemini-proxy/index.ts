  ...
  try {
    ...
    switch (type) {
        case 'generateContent': {
            ...
            // **FIX**: Manually construct a plain object to ensure serializability.
            // The Gemini SDK response object can be complex and cause issues with JSON.stringify in Deno.
            // This ensures we only send back clean, necessary data.
            const plainCandidates = response.candidates?.map(candidate => JSON.parse(JSON.stringify(candidate)));

            result = {
              text: response.text ?? '', // Ensure text is always a string, not undefined
