...
const WorkingShotstackStudio: React.FC = () => {
  console.log('🎬 WorkingShotstackStudio component loaded!');
  ...
  // Refs for DOM elements - CRITICAL for proper initialization
  ...
  const waitForHosts = async (): Promise<void> => {
    ...
    for (let i = 0; i < 60; i++) { // Try for up to 1 second (60 frames)
  ...
  // Initialize when both elements are ready - EXACTLY like the old working version
  useEffect(() => {
    console.log('🎬 WorkingShotstackStudio useEffect triggered:', { 
      initialized, 
      canvasReady,
      timelineReady,
      isLoading
  ...
  const initializeEditor = async () => {
    try {
      ...
      // 3. Create a canvas to display the edit - EXACTLY like the old working version
      ...
      // 4. Load the base template first - EXACTLY like the old working version
      console.log('📄 Loading base template...');
      await editInstance.loadEdit(template);
      console.log('✅ Base template loaded');
      
      // 5. Add keyboard controls - EXACTLY like the old working version
      console.log('⌨️ Creating Controls component...');
      const controls = new Controls(editInstance);
      await controls.load();
      console.log('✅ Controls component loaded');

      // 6. Add timeline for visual editing - EXACTLY like the old working version
      ...
      // Set up event listeners - EXACTLY like the old working version
