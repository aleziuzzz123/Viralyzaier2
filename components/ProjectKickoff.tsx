            ...
            <div className="space-y-6 bg-gray-800/50 p-8 rounded-2xl border border-gray-700">
                ...
                <div>
                    ...
                    <div className="flex items-center justify-between mt-2 bg-gray-900 rounded-lg p-3">
                        <p className="text-sm text-gray-400">Generate visual ideas for your script?</p>
                        <button onClick={() => setGenerateMoodboard(!generateMoodboard)} className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${generateMoodboard ? 'bg-indigo-600' : 'bg-gray-600'}`}>
                            <span className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${generateMoodboard ? 'translate-x-6' : 'translate-x-1'}`}/>
                        </button>
                    </div>
                </div>
